from campeonato.db import database
from db.database import DataBase
from model.models import Categoria, Escola, Atleta, Podio
...
class SistemaCBJJ:
    ...
    def cadastrar_atleta(self, atleta: Atleta):
        cur = self.db.cursor()
        cur.execute("""
            INSERT INTO atletas (nome, email, idade, categoria_idade, altura, peso, data_nascimento, telefone, observacoes, cad_unico, escola_id, categoria_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def listar_participantes(self):
        ...
        if not participantes:
            print("Nenhum atletas cadastrado ainda.")
            return
